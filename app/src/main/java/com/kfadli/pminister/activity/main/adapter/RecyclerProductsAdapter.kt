...
class RecyclerProductsAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): ProductHolder {
        return ProductHolder(parent?.context,
    ...
    class ProductHolder(val context: Context?, itemView: View?) : RecyclerView.ViewHolder(itemView), OnClickListener {
        ...
        fun bind(product: ProductsItem) {
            ...
            itemView.new_price_txt.text = "${currencyFormat().format(product.newBestPrice)} ${context?.getString(R.string.new_product)}"
            itemView.used_price_txt.text = """${currencyFormat().format(product.bestPrice)} ${context?.getString(R.string.used_product)}"""
            itemView.picture_img.loadUrl(items[0]!!.url!!)
            itemView.reviews_txt.text = "${product.nbReviews.toString()} ${context?.getString(R.string.reviews)}"
